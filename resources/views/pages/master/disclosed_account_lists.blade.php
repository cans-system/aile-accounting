<x-layout>
      ...
      @foreach ($lists as $list)
        <tr>
          ...
          <td>
            ...
            delete-action="/master/disclosed_account_lists/{{ $list->id }}" />
          </td>
        </tr>
      @endforeach
    </tbody>
  ...
  @foreach ($lists as $list)
    <x-modal id="editModal{{ $list->id }}" title="編集">
      <form action="/master/disclosed_account_lists/{{ $list->id }}" method="post">
        @csrf
        @method('PUT')
        <div class="mb-3">
  ...
  <x-modal id="createModal" title="新規作成">
    <form action="/master/disclosed_account_lists" method="post">
      @csrf
      <div class="mb-3">
        <label class="form-label">開示科目名称</label>
